...
export function mapDamageDraftToReport(
  ...
  if (draft.shortDescription) {
    builder.addSection({
      ...
      blocks: [{ kind: "PARAGRAPH", text: draft.shortDescription }],
    });
  }
  ...
  if (events.length === 0) {
    ...
    events.forEach((e) => {
      ...
      if (e.description) {
        timelineBlocks.push({ kind: "PARAGRAPH", text: e.description });
      }
      const codes = (e.evidenceNumbers || [])
        .map((num) => remappedEvidence.find((x) => x.evidenceNumber === num)?.evidenceCode)
        .filter(Boolean) as string[];
      if (codes.length > 0) {
        timelineBlocks.push({ kind: "PARAGRAPH", text: `Dokumentasjon: ${codes.join(", ")}` });
      }
    });
  }
  ...
  if (draft.observations) {
    ...
    technicalBlocks.push({ kind: "PARAGRAPH", text: draft.observations });
  }
  if (draft.technicalAssessment) {
    technicalBlocks.push({ kind: "HEADING", text: "Tekniske vurderinger", level: 3 });
    technicalBlocks.push({ kind: "PARAGRAPH", text: draft.technicalAssessment });
  }
  ...
  if (measurementsText) {
    ...
    technicalBlocks.push({ kind: "PARAGRAPH", text: measurementsText });
  }
  if (draft.drawings) {
    technicalBlocks.push({ kind: "HEADING", text: "Tegninger", level: 3 });
    technicalBlocks.push({ kind: "PARAGRAPH", text: draft.drawings });
  }
  if (draft.calculations) {
    technicalBlocks.push({ kind: "HEADING", text: "Beregninger", level: 3 });
    technicalBlocks.push({ kind: "PARAGRAPH", text: draft.calculations });
  }
  ...
  builder.addSection({
    ...
    blocks: [
      ...
      { kind: "PARAGRAPH", text: draft.conclusion || "Ingen konklusjon er utfylt." },
    ],
  });
